...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<String> findAllBookTitlesWithAgeRestriction() {
        return null;
    }
    ...
    @Override
    public List<String> findAllGoldenBookTitlesWithLessThan5000Copies() {
        return bookRepository.findAllByEditionTypeAndCopiesLessThan(EditionType.GOLD,5000).stream().map(Book::getTitle)
                .collect(Collectors.toList());
    }
    ...
    public List<String> findAllBookTitlesWithPriceLessThan5MoreThan40() {
        return bookRepository
                ...
                .stream().map(book -> String.format("%s - $ %.2f", book.getTitle(),
                        book.getPrice()))
                .collect(Collectors.toList());
    }
    ...
    public List<String> findNotReleasedBookTitlesInYear(int year) {
        ...
        return bookRepository.findAllByReleaseDateBeforeOrReleaseDateAfter(lower, upper).stream()
    ...
    public List<String> findAllBookTitlesWhereTitleContainsStr(String str) {
        return bookRepository.findByTitleContains(str).stream().map(Book::getTitle).collect(Collectors.toList());
    }

    @Override
    public List<String> findAllTitlesWithAuthorsLastNameStartsWith(String startStr) {
